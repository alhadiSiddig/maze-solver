...
from maze_solver.view.primitives import Point , Rect , Text , Polyline  ,  tag 


ROLE_EMOJI = {
    Role.ENTERANCE :  "\N{pedestrian}" , 
...
class SVGRenderer: 
    ...
    def render(self , maze : Maze , solution:Solution |None = None ) -> SVG : 
        ...
        return SVG (
               tag('svg' , self._get_body(maze,solution) , 
...
def arrow_marker() -> str : 
    return tag(
        'defs' , tag(
            'marker' , tag( 
                'path' , d = 'M 0,0 L 10,5 L 0,10 2,5 z' , 
                fill = 'red' ,
                fill_opacity = '50%'
            ), 
            id = 'arrow' ,
